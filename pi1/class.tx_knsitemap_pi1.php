...
class tx_knsitemap_pi1 extends tslib_pibase {
	...
	function outputSubpages() {
		...
		//	$x .= '<a href="?id='.$row['uid'].'" title="'.$titleTag.'" target="'.$this->iconArray[$row['doktype']]['target'].'"><img src="typo3conf/ext/kn_sitemap/res/'.$this->iconArray[$row['doktype']]['icon'].'" width="16" height="16" title="'.$titleTag.'" alt="'.$titleTag.'" />'.$row['title'].'</a></li>';
		...
		//$this->pi_linkToPage($this->linkText,$this->toolkitURL.'?'.$parameters, '_blank', array())
		//	$this->closeJSCode .= 'var orig'.$row['uid'].' = "'.str_replace('"','\\"',$x).'";'."\n";
	 ...
	 * Erzeugt title-Tag
	 *
	 * @param int	Konfig-Parameter
	 * @param Array Datensatz
	 * @return String gerenderter Title-Tag
	 */
